...
def iterative_prune_and_finetune(p, df, c_epoch, t_epoch, lr, target_flag ,store_path):
    ...
    ###### step_scheduler for ImageNet ResNet combination #####
    def learningrateprovider(epoch, first_step=30, second_step=60, third_step=90,base_lr=lr):
        if epoch > third_step:
            lear = base_lr/1000
        elif epoch > second_step:
            lear = base_lr / 100
    ...
    for e in range(c_epoch+1, t_epoch + 1):
        ...
        # optim = torch.optim.SGD(net.parameters(), lr=lr, momentum=0.875, weight_decay=1 / 32768)
