...
def iterative_prune_and_finetune(p, df, c_epoch, t_epoch, lr, target_flag ,store_path):
    ...
    # def learningrateprovider(epoch, warmup_epochs=0, max_epoch_for_cosine=150, base_lr=0.05, final_lr=1e-6,
    ...
    def learningrateprovider(epoch, base_lr=0.04):
        pow_factor = np.int(epoch / 20)
        lear = base_lr / np.power(2, pow_factor)
        return lear
