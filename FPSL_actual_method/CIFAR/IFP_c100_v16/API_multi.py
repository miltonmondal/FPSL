...
class Models:
    ...
    def net(self):
        ...
        if self._model == 'ResNet':
            net = self._ResNet_(
                self._num_layers, self._num_class, self._pretrained, with_conv_multipliers=self._with_conv_multipliers
            )
            return net
    ...
    class _Agnostic_:
        ...
        def distill_training(
            self, dataset, eval_freq, epoch, teacher, preprocessFn=None, loss_in_db=False, final_eval=True
        ):
            collate_loss, correct_predictions, total_processed = (
                torch.zeros(1).cuda(),
                torch.zeros(1).cuda(),
    ...
    class _ResNet_(nn.Module, _Agnostic_):
        ...
        def __init__(self, num_layers, num_class, pretrained=False, first_block=False, with_conv_multipliers=False):
            super().__init__()
        ...
        def dry_run(self, dataset, num_iterations=10):
            if self._optim is None:
                raise ConnectionError('Optimizer not connected. Use attach_optimizer to connect optimizer.')
            iteration = 0
            for data, labels in dataset.train_images:
        ...
        def get_features(
            self, dataset, num_batches, after_layer, layer_type='conv', return_type='tensor', verbose=False
        ):
        ...
        def get_weights(self, layer, filter=None, grad=False, next_conv=False):
            curr_conv_idx = self._idx[layer][0]
            if next_conv:
                flag_found = False
                for i in range(curr_conv_idx + 1, curr_conv_idx + 10):
                ...
                required_layer = self.get_layer_from_tree(curr_conv_idx)
            ...
            def prune_conv_layer(internal_idx, filter, in_out):
                ...
                if in_out == 'out':
                    ...
                    try:
                        ...
                        conv0.__init__(
                            conv0_in_channels, conv0_out_channels - 1, conv0_kernel_size, conv0_stride, conv0_padding
                        )
                    except:
                    ...
                    try:
                        ...
                        conv0.__init__(
                            conv0_in_channels - 1, conv0_out_channels, conv0_kernel_size, conv0_stride, conv0_padding
                        )
                    except:
                        conv0.__init__(
                            conv0_in_channels - 1,
        ...
        def watch_filter(self, layer, filter=None, net=None):
            ...
            with torch.no_grad():
                if filter is not None:
                    print(required_layer.weight[filter].sum())
                    print(required_layer.bias[filter])
                else:
                    print(required_layer.weight.sum(dim=(1, 2, 3)))
    ...
    class _VGG_(nn.Module, _Agnostic_):
        ...
        def get_features(
            ...
            assert layer_type in ['conv', 'bn', 'relu'], (
                'layer_type must be either [' + ' '.join(['conv', 'bn', 'relu']) + ']'
            )
            assert return_type in ['tensor', 'mean'], (
                'return_type must be either [' + ' '.join(['tensor', 'mean']) + ']'
            )
            ...
            def prune_linear_layer(idx, filter, rc, num_units=1):
                ...
                if rc == 'row':
                    ...
                    for _ in range(num_units):
                        ln_target_weight = delete_index(ln_target_weight, filter, dim=1)
                    ln_target_bias = ln_bias
