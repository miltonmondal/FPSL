...
def filter_norm_present_next_layer(net, path_epochwise):
    ...
    for l in range(net.max_layers() - V.ig_l):
        ...
        weight_l_next, bias_l_next = net.get_weights(l+1)
        ...
        normal_p_n = p_n / weight_l.shape[0]
        normal_n_n = n_n / weight_l.shape[0]
...
def iterative_prune_and_finetune(p, df, c_epoch, t_epoch, lr, target_flag ,store_path):
    ...
    print("baseline net accuracy : ", te_acc_org)
    # net = api.Models(model=V.model_str, num_layers=V.n_l, num_transition_shape=1 * 1, num_linear_units=512,
    #                  num_class=V.n_c).net()
    # print("net accuracy at initial condition", net.evaluate(V.dataset, train_images=False))
    ...
    for e in range(c_epoch+1, t_epoch + 1):
        ...
        net.start_training(V.dataset, eval_freq=num_itr_per_epoch, epoch=1)
    ...
    if last_epoch == t_epoch:
        ...
        row2 = [round((macs_pruned / 1e6), 4), round(per_red_FLOPs, 4), round((params_pruned / 1e6), 4),
                round(per_red_param, 4),
                round(acc_train, 4), round(train_top5, 4), round(te_acc_org[0], 4), round(te_acc_org[1], 4),
                round(acc_test, 4), round(test_top5, 4)]
